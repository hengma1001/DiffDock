...
def main(args):
    ...
    try:
        log_file = f"{args.out_dir}/failed_ligands.txt"
        bad_ligands_fh = open(log_file, "w", buffering=1)
    except Exception as e:
        print(f"Cannot open bad-ligand error log {log_file}: {e}", file=sys.stderr)
        sys.exit(1)
